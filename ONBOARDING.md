...
`kaggle_environments` 内置了一个 HTML 播放器，可以生成完整的交互式回放文件，在浏览器中观看对战过程（行星移动、舰队轨迹、分数曲线等）。
...
# v13 vs v12，seed=42，全程 500 步，生成后自动打开浏览器
python3.12 scripts/replay.py

# 指定版本 / seed / 步数
python3.12 scripts/replay.py --a v13 --b v11 --seed 7 --steps 200

# vs 内置 random（用于快速 smoke）
python3.12 scripts/replay.py --a v13 --b random --seed 0

# 生成但不打开浏览器（CI 场景）
python3.12 scripts/replay.py --a v13 --b v9 --seed 3 --no-open

# 自定义输出路径
python3.12 scripts/replay.py --a v13 --b v12 --out /tmp/my_game.html
```

回放文件默认保存到 `replays/<a>_vs_<b>_seed<N>.html`（约 5 MB），用 Chrome/Safari 打开即可。

### 6.2 Jupyter Notebook 内联观看
...
def load(ver):
    spec = importlib.util.spec_from_file_location(ver, f"submission_{ver}.py")
    m = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(m)
    return m.agent
...
for _ in range(50):
    if env.done: break
    env.step([lambda o,c,f=a: f(o,c), lambda o,c,f=b: f(o,c)])

# Notebook 内直接渲染（需要 ipywidgets / JupyterLab）
env.render(mode="ipython")
```

### 6.3 Kaggle 官网竞赛页面回放

提交到 Kaggle 后，在比赛的 **Leaderboard → Episodes** 页面可以看到最新对战的完整录像，支持逐帧播放。这是观察 bot 在真实排名对局中表现的最好方式。

---

## 八、理解 submission_v13.py 的核心结构
