;;; =============================================================================
;;; 中文注释副本 — 对应 ../../src/alpha-beta.lisp
;;; 浅层 α–β 与 MOVE 宽度 *WIDTHS*、DEFERRED-ORDERS 协同。
;;; =============================================================================

(in-package :planet-wars)

;;; 极小极大递归： MAYBE-EVALUATE-STATE 为叶；否则 LIST-ACTIONS 枚举、CALL-WITH-ACTION 下潜。
...
;;; 每层保留的候选 MOVE 个数向量；NIL 则用默认推导。
(defparameter *widths* nil)

;;; 「尚未落到盘的」挂单拆分用动态变量。
(defvar *deferred-orders* nil)

;;; 将 ORDERS 按 TURN 切成已生效与仍为未来挂单两段。
...
;;; 体身内只看到应已执行的 PAST-ORDERS；未来单保留在 *DEFERRED-ORDERS*。
...
;;; 达最大深度则 EVALUATE/FULL-ATTACK；否则返回 NIL 继续展开。吞 FUTURE-IMPOSSIBLE 用 CONTINUE。
...
;;; 从 GENERATE-AND-SCORE-MOVES 取前 (AREF *WIDTHS* DEPTH) 个 move（或深度处宽 0 则仅空 move）。
...
;;; WITH-ORDERS 应用 ORDERS；下潜时 *DEPTH*、*TURN* 按对手先后更新。
...
;;; 入口：绑定 *WIDTHS*（截断到合理上界），启动根层 α–β；根层 NEW-BEST 写 *BEST-MOVE-SO-FAR*。
