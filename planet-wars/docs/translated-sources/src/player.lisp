;;; =============================================================================
;;; 中文注释副本 — 对应 ../../src/player.lisp
;;; Bot 棋子力：超时/FUTURE/SURPLUS/评估/地平线/走子生成。详见 handwritten/detail/player/。
;;; =============================================================================
...
;;; 超时兜底：根层已见最优 (MOVE . SCORE)。
(defvar *best-move-so-far*)

;;; 仅在 DEPTH=0 且更优时更新 BEST。
...
;;; Allegro：MP:WITH-TIMEOUT 包装。
#+allegro
;;; 超时返回 CAR *BEST-MOVE-SO-FAR* 或空表。
...
;;; SBCL：SB-EXT:WITH-TIMEOUT + HANDLER-CASE。
#+sbcl
;;; 超时返回 CAR *BEST-MOVE-SO-FAR* 或空表。
(defmacro with-best-move-on-timeout (timeout &body body)
  `(let ((*best-move-so-far* nil))
...
;;; 单星未来：OWNERS/N-SHIPS 向量 + BALANCE。
(defclass future ()
...
;;; 与 EXECUTE-ORDER 不一致时 CERROR 继续。
...
;;; 读局、算 HORIZON、TRUNCATE、WITH-ORDERS 空层。
