;;; =============================================================================
;;; 中文注释副本 — 对应 ../../src/timer.lisp
;;; SB-IMPL 包：优先队列调度与 WITH-TIMEOUT backport。（仅 SBCL）
;;; =============================================================================
...
;;; 堆父/子下标与 HEAPIFY/INSERT/EXTRACT。
(defun heap-parent (i)
  (ash (1- i) -1))
...
;;; TIMER：NAME/FUNCTION/EXPIRE-TIME/REPEAT/THREAD/INTERRUPT/CANCEL。
(defstruct (timer
             (:conc-name %timer-)
...
;;; 用户 API：THREAD 控制在新线程或 INTERRUPT 既有线程执行。
...
;;; 墙钟超时宏（与 PLAYER 中 HANDLER-CASE 配对）。
