"""Orbit Wars v15 - v14 + precision fleet routing overhaul.

Root cause of "fleets flying off-screen":
  safe_aim used `src + cos(angle)*spd*eta` as the endpoint for OOB/sun checks.
  This is the fleet's *travel vector*, not the target's actual future position.
  For orbiting planets the two diverge significantly, causing the angle to be
  computed correctly but the OOB check to fire on the wrong point, then the
  eta-shrink loop to shorten the flight without re-aiming, producing a fleet
  that flies in the right direction but stops short (or overshoots the board).
...
def safe_aim(state: GameState, src: Planet, dst: Planet, ships: int) -> Tuple[float, int]:
    ...
    v16 fix: the inner eta-shrink loop previously decremented e to 0 before the
    outer clearance-tracking block reset it to max(1,...), which could still
    produce an OOB endpoint when the reset e=1 ray also left the board.
    Now the two passes are unified: one loop per angle, shrinking e from eta
    down to 1, returning immediately on a clean shot, or recording the best
    clearance candidate if no clean shot exists for that angle.
    """
    spd = fleet_speed(max(1, ships), state.max_speed)
    ...
    def in_board(x: float, y: float) -> bool:
        return 0.5 <= x <= BOARD - 0.5 and 0.5 <= y <= BOARD - 0.5

    def ray_endpoint(a: float, e: int) -> Tuple[float, float]:
        return (src.x + math.cos(a) * spd * e,
                src.y + math.sin(a) * spd * e)

    # Fast path: ideal intercept point is safe AND the straight-line flight
    # endpoint (src + cos(a)*spd*eta) is also inside the board.
    ex0 = src.x + math.cos(angle0) * spd * eta
    ey0 = src.y + math.sin(angle0) * spd * eta
    if in_board(tx, ty) and in_board(ex0, ey0) and not segment_hits_sun(src.x, src.y, tx, ty):
        return angle0, eta

    deltas = (0.0, 0.12, -0.12, 0.22, -0.22, 0.34, -0.34, 0.50, -0.50,
    ...
    for delta in deltas:
        a = angle0 + delta
        # Single unified pass: shrink e from eta down to 1.
        for e in range(max(1, eta), 0, -1):
            ...
            cl = point_segment_distance(SUN_X, SUN_Y, src.x, src.y, ex, ey) - (
                SUN_RADIUS + 2.5)
            if cl > 0:
                # Clean shot found for this angle/eta combo - take it immediately.
                return a, e
            # In-board but clips sun: track best clearance candidate and
            # keep searching smaller e values that might clear the sun.
            if cl > best_cl:
                best_cl = cl
                best_a = a
                best_e = e
...
class PlanArbiter:
    ...
    def _emit(self, sid: int, did: int, ships: int, urgent: bool) -> bool:
        ...
        # v17 hard safety gate: reject if the flight path still hits the sun
        # or the endpoint is out of board. This is the last line of defense.
        spd = fleet_speed(send, state.max_speed)
        fx = src.x + math.cos(angle) * spd * eta
        fy = src.y + math.sin(angle) * spd * eta
        if not (0.5 <= fx <= BOARD - 0.5 and 0.5 <= fy <= BOARD - 0.5):
            return False
        if segment_hits_sun(src.x, src.y, fx, fy):
            return False
