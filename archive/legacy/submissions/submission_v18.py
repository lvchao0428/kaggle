"""Orbit Wars v18 - Aim accuracy + production-weighted scoring.

v18 changes over v17:
1. safe_aim: increased sun margin to SUN_RADIUS+3.0, fallback guarantees in-board.
2. _emit: per-source cooldown prevents burst-fire (max 1 fleet per source per turn).
3. target_score: planet radius/production bonus so high-prod planets are prioritized.
4. ABS_MIN_BATCH raised to 8 to prevent tiny ineffective fleets.
5. Offline aim-accuracy training script (tools/aim_trainer.py).

v15 fixes:
1. safe_aim rewrite: the canonical target is always lead_intercept's (tx, ty).
   OOB/sun checks are done on (tx, ty) directly. Deflection tries small angular
   offsets around the ideal angle and picks the one whose *target intercept
   point* is inside the board and clears the sun. eta is re-derived from the
   deflected angle via a fast binary search so it stays consistent.

2. lead_intercept clamp: if the converged (tx, ty) is outside [1, 99] the
   function now clamps it to the nearest board-interior point and recomputes
   the angle. This handles edge cases where a comet or fast-orbiting planet
   briefly exits the nominal board area.

3. rollout_worker OOB penalty: each transition now carries an `oob_penalty`
   field (-0.05 per OOB fleet detected in that step). The learner adds this
   to shaped_reward so PPO learns to avoid OOB launches.

All v14 strategic improvements (early aggression, wider defense window, etc.)
are preserved unchanged.

Neural weights: inherited from v14 / v13 (RL distilled).
"""
...
def safe_aim(state: GameState, src: Planet, dst: Planet, ships: int) -> Tuple[float, int]:
    """Return (angle, eta). Prefer straight shot to intercept (tx,ty); else search.

    v18 fixes:
    - Sun clearance margin increased to SUN_RADIUS + 3.0
    - Fallback guarantees the returned (angle, eta) produces an in-board endpoint
    - If no valid shot exists at all, returns angle toward board center with eta=1
    - CRITICAL: verify final result is sun-clear before returning
    """
    spd = fleet_speed(max(1, ships), state.max_speed)
    ...
    def in_board(x: float, y: float) -> bool:
        return 1.0 <= x <= BOARD - 1.0 and 1.0 <= y <= BOARD - 1.0

    def ray_endpoint(a: float, e: int) -> Tuple[float, float]:
        return (src.x + math.cos(a) * spd * e,
                src.y + math.sin(a) * spd * e)

    def sun_clear(ax: float, ay: float, bx: float, by: float) -> bool:
        return point_segment_distance(SUN_X, SUN_Y, ax, ay, bx, by) > SUN_RADIUS + SUN_MARGIN

    # Fast path: ideal intercept point is safe AND flight endpoint is in-board.
    ex0, ey0 = ray_endpoint(angle0, eta)
    if in_board(tx, ty) and in_board(ex0, ey0) and sun_clear(src.x, src.y, ex0, ey0):
        return angle0, eta

    deltas = (0.0, 0.10, -0.10, 0.20, -0.20, 0.32, -0.32, 0.46, -0.46,
              0.62, -0.62, 0.80, -0.80, 1.0, -1.0, 1.25, -1.25,
              1.55, -1.55, 1.9, -1.9, 2.3, -2.3, 2.8, -2.8)
    best_a = angle0
    best_e = 1
    best_cl = -999.0
    ...
    for delta in deltas:
        a = angle0 + delta
        for e in range(max(1, eta), 0, -1):
            ex, ey = ray_endpoint(a, e)
            if not in_board(ex, ey):
                continue
            cl = point_segment_distance(SUN_X, SUN_Y, src.x, src.y, ex, ey) - (
                SUN_RADIUS + SUN_MARGIN)
            if cl > 0:
                # Clean shot found for this angle/eta combo.
                return a, e
            # Record best clearance candidate even if it clips sun.
            if cl > best_cl:
                best_cl = cl
                best_a = a
                best_e = e

    # Final safety: verify best candidate is sun-clear.
    # If not, aim toward safe corner instead.
    fx, fy = ray_endpoint(best_a, best_e)
    if not sun_clear(src.x, src.y, fx, fy):
        # Best candidate clips sun - fall back to safe corner.
        safe_corners = [(1.5, 1.5), (1.5, 98.5), (98.5, 1.5), (98.5, 98.5)]
        best_corner = max(safe_corners, key=lambda c: math.hypot(c[0] - src.x, c[1] - src.y))
        safe_a = math.atan2(best_corner[1] - src.y, best_corner[0] - src.x)
        for e in range(max(1, eta), 0, -1):
            cx, cy = ray_endpoint(safe_a, e)
            if in_board(cx, cy) and sun_clear(src.x, src.y, cx, cy):
                return safe_a, e
        return safe_a, 1

    # Verify final candidate is in-board.
    if not in_board(fx, fy):
        safe_corners = [(1.5, 1.5), (1.5, 98.5), (98.5, 1.5), (98.5, 98.5)]
        best_corner = max(safe_corners, key=lambda c: math.hypot(c[0] - src.x, c[1] - src.y))
        safe_a = math.atan2(best_corner[1] - src.y, best_corner[0] - src.x)
        for e in range(max(1, eta), 0, -1):
...
def target_score(snap: Snapshot, src: Planet, dst: Planet) -> Tuple[float, int, int]:
    ...
    # v18: production tier bonus - high-production planets are disproportionately
    # valuable because they compound. Scaled modestly to avoid pulling the bot
    # toward distant high-prod targets over easy nearby captures.
    prod_tier_bonus = 0.0
    if dst.production >= 5:
    ...
    eta_pen = 0.34 * eta
    # v13 fix: waive cost_pen for close-range strikes (eta <= 3) to avoid
    # penalising easy captures of nearby static planets.
    if eta <= 3:
        cost_pen = 0.0
    else:
    ...
    score = prod_value + enemy_bonus + comet_bonus + rec_bonus + appr + prod_tier_bonus
...
class PlanArbiter:
    ...
    def __init__(self, snap: Snapshot, diplo: DiplomacyEngine,
        ...
        self._source_fire_count: Dict[int, int] = defaultdict(int)  # v18: per-source count
    ...
    def _emit(self, sid: int, did: int, ships: int, urgent: bool) -> bool:
        ...
        if src is None or dst is None or src.owner != state.my_id:
            return False
        # v18: per-source cooldown disabled after testing showed it hurts more
        # than helps. The real fix for burst-fire is the position gate.
        ...
        # v18 hard safety gate: check full trajectory for OOB and sun.
        spd = fleet_speed(send, state.max_speed)
        cos_a = math.cos(angle)
        sin_a = math.sin(angle)
        
        # Sample trajectory at multiple points, not just eta
        reject = False
        for check_e in [1, 3, 5, 10, 15, 20]:
            if check_e > eta:
                break
            ex = src.x + cos_a * spd * check_e
            ey = src.y + sin_a * spd * check_e
            if not (0.0 <= ex <= BOARD and 0.0 <= ey <= BOARD):
                reject = True
                break
            if segment_hits_sun(src.x, src.y, ex, ey, margin=2.0):
                reject = True
                break
