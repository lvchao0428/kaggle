"""Orbit Wars v14 - v13 + aggressive early game + wider defense window.

Key improvements over v13:

1. Early-game aggression: reserve_growth_mul drops from 4 to 2 in early phase,
   giving planets meaningful surplus from turn 1. mode_order gains aggro/counter
   in early phase so nearby enemy planets are contested immediately.

2. Wider proactive defense window: threat_horizon now triggers at eta <= 8
   (was 3), giving more time to reinforce high-production planets under threat.
   DefensePlanner proactive window also expanded to 8 turns.

3. Lower source contribution threshold: _build_capture_plan now allows any
   source with avail >= ABS_MIN_BATCH to join a capture group (was max(5, prod*2)),
   enabling small-production planets to contribute to coordinated captures.

4. UrgentHighProdPlanner unblocked in early phase for close targets (eta <= 6):
   urgent_attack_ratio still high for distant targets, but immediately adjacent
   high-production neutrals/enemies are captured without waiting for mid-phase.

Neural weights inherited from v13 (v12 real-train PPO distillation).
"""
