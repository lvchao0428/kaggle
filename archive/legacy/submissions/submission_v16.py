...
def safe_aim(state: GameState, src: Planet, dst: Planet, ships: int) -> Tuple[float, int]:
    ...
    # Fast path: ideal intercept point is safe.
    if in_board(tx, ty) and not segment_hits_sun(src.x, src.y, tx, ty):
        return angle0, eta
    ...
    for delta in deltas:
        ...
        for e in range(max(1, eta), 0, -1):
            ...
            if cl > 0:
                ...
                return a, e
            # In-board but clips sun: track best clearance candidate.
            if cl > best_cl:
                best_cl = cl
                best_a = a
                best_e = e
            break  # Only record the largest e that is in-board for this angle.
