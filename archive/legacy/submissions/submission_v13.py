"""Orbit Wars v13 - v12 + Phase 1 bug fixes.

Three targeted fixes over v12:

1. safe_aim OOB: fallback loop now shrinks eta until endpoint stays inside
   the [0.5, 99.5] board boundary, preventing fleets flying off-screen.

2. Near-range cost_pen waiver: targets with eta <= 3 bypass the cost_penalty
   so adjacent static planets are captured promptly without economic penalty.
   Also, is_safe_investment is bypassed for group_eta <= 3 in expand mode.

3. Proactive defense: Snapshot.threat_horizon tracks the earliest in-flight
   enemy fleet ETA per friendly planet. DefensePlanner uses this to reinforce
   high-production planets before the threat materialises (eta <= 3 window).
   _reserve also doubles growth_lock for imminently threatened high-prod stars.

Neural weights (`_NEURAL_WEIGHTS_B64`) are inherited from v12 (distilled from
the real-train PPO run).
"""
...
def safe_aim(state: GameState, src: Planet, dst: Planet, ships: int) -> Tuple[float, int]:
    ...
    for delta in deltas:
        ...
        e = eta
        ex, ey = endpoint(a, e)
        # If endpoint is OOB, shrink eta until it fits in board or hits minimum.
        while not (0.5 <= ex <= BOARD - 0.5 and 0.5 <= ey <= BOARD - 0.5) and e > 1:
            e -= 1
            ex, ey = endpoint(a, e)
        if not (0.5 <= ex <= BOARD - 0.5 and 0.5 <= ey <= BOARD - 0.5):
            continue
...
class Snapshot:
    ...
    def _reserve(self, p: Planet) -> int:
        ...
        # v13: if an enemy fleet will arrive within 3 turns AND this planet
        # has high production, keep extra buffer to defend it proactively.
        horizon_eta = self.threat_horizon.get(p.id, 999)
        if horizon_eta <= 3 and p.production >= 3:
            base = max(base, p.ships + p.production * 4)
        return base
...
class DefensePlanner:
    ...
    def plan(snap: Snapshot) -> Plan:
        ...
        # Combine reactive (net_threat > 0) and proactive (threat_horizon <= 3
        # on high-production planets) targets.
