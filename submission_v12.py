"""Orbit Wars v12 — v11 framework + RL-distilled NeuralVal weights.

Identical to submission_v11.py in structure; the only change is that
`_NEURAL_WEIGHTS_B64` now contains weights distilled from a PyTorch PPO
self-play run (see `tools/learner.py`, `tools/distill_to_numpy.py`).

The RL pipeline trained a 31-feature PolicyValueNet (state + plan-summary)
via PPO self-play, then distilled the value head into a v11-compatible
14 -> 64 -> 32 -> 1 NumPy MLP — so v12 is a pure weight swap; the
