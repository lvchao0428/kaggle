...
def safe_aim(state: GameState, src: Planet, dst: Planet, ships: int) -> Tuple[float, int]:
    ...
    # v15: check sun using segment to predicted intercept, not a mismatched ray length.
    ...
    for delta in deltas:
        ...
        while e >= 1:
            ...
            if in_board(ex, ey) and not segment_hits_sun(src.x, src.y, ex, ey):
                return a, e
            e -= 1
        e = max(1, min(eta, 80))
        ex, ey = ray_endpoint(a, e)
        while not in_board(ex, ey) and e > 1:
            e -= 1
            ex, ey = ray_endpoint(a, e)
...
def _build_capture_plan(snap: Snapshot, mode: str,
    ...
    for _, dst in ranked[:MAX_TARGETS_PER_PLAN]:
        ...
        # v13: also bypass for very close targets (group_eta <= 3) - never
        # block trivial captures of adjacent static planets.
