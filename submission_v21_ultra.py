"""Orbit Wars v21_ultra - v20 lineage + wide NeuralVal (192→96→1). Heavier per-step cost; prefer offline RL teacher + distill to lite/pro for submit.
...
class NeuralVal:
    ...
    def __init__(self):
        ...
        self.W1 = rng.normal(0, 0.2, (192, self.N_FEAT)).astype(np.float32)
        self.b1 = np.zeros(192, dtype=np.float32)
        self.W2 = rng.normal(0, 0.2, (96, 192)).astype(np.float32)
        self.b2 = np.zeros(96, dtype=np.float32)
        self.W3 = rng.normal(0, 0.2, (1, 96)).astype(np.float32)
